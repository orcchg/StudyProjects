...
const int SORT_LIMIT = 10;  // Chosen after 15 Time Limit tests 450
...
//  segment_border_type_array->resize(number_points_n * 2);
  int pair_index = 0;
    ...
    try {
      ...
      /*segment_border_type_array->at(pair_index)*/pair.coordinate = 
          segment_of_current_point.left_border;
      /*segment_border_type_array->at(pair_index)*/pair.type = LEFT_BORDER_TYPE;
      //++pair_index;
			segment_border_type_array->push_back(pair);
 
      /*segment_border_type_array->at(pair_index)*/pair.coordinate =
          segment_of_current_point.right_border;
      /*segment_border_type_array->at(pair_index)*/pair.type = RIGHT_BORDER_TYPE;
      //++pair_index;
			segment_border_type_array->push_back(pair);
 ...
 if (begin < end) {
  ...
  if (begin < iterator_last) {
    if (iterator_last - begin > SORT_LIMIT) {
      ...
      insertionSortArrayOfPairs(begin, iterator_last, array_of_pairs);
    }
  }
  if (iterator_first < end) {
    if (end - iterator_first > SORT_LIMIT) {
      ...
      insertionSortArrayOfPairs(iterator_first, end, array_of_pairs);
    }
  }
 }
}
...
//		Output_Pair(segment_border_type_array);
    cout << segment_border_type_array.size() << " SIZE" << endl;
 
    quickSortArrayOfPairs_simple(
        0, 
        /*2 * number_points_n - 1*/segment_border_type_array.size() - 1, 
        &segment_border_type_array);
