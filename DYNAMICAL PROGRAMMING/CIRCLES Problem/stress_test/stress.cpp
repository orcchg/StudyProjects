...
const int SORT_LIMIT = 0; // Chosen after 15 Time Limit tests 450
...
/****************************************************************************/
//-----------------------------------------------------------------------------
Segment calculateSegment_LAST(const Point& point, double radius, bool* error_flag) {
  Segment segment;
  if (radius < static_cast<double>(fabs(point.coordinate_Y))) {
    *error_flag = true;
    return segment;
...
//-----------------------------------------------------------------------------
void fillArrayOfSegmentBorderAndType_LAST(
    int number_points_n,
    const vector<Point>& points_array,
    double radius,
    vector<Pair>* segment_border_type_array) {
  ...
  for (int point_index = 0; point_index < number_points_n; ++point_index) {
      Segment segment_of_current_point = 
          calculateSegment_LAST(points_array[point_index], radius, &error_flag);
      if (error_flag) {
        error_flag = false;
        continue;
      }
...
//-----------------------------------------------------------------------------
void insertionSortArrayOfPairs_LAST(
    int begin,
    int end,
    vector<Pair>* segment_border_type_array) {
...
//-----------------------------------------------------------------------------
int partition_LAST(int begin, int end, vector<Pair>* array_of_pairs) {
  int pivot_index = end;
  Pair pivot = array_of_pairs->at(pivot_index);
  int iterator = begin - 1;
  for (int array_iterator = begin; array_iterator < end; ++array_iterator) {
...
//-----------------------------------------------------------------------------
void quickSortArrayOfPairs_LAST(
    int begin,
    int end,
    vector<Pair>* segment_border_type_array) {
  ...
  int pivot_place;
  if (begin < end) {
    pivot_place = partition_LAST(begin, end, segment_border_type_array);
 
    if (end - begin > SORT_LIMIT) {
      quickSortArrayOfPairs_LAST(
          begin, 
          pivot_place - 1, 
          segment_border_type_array);
    } else {
      insertionSortArrayOfPairs_LAST(
          begin,
          pivot_place - 1,
          segment_border_type_array);
    }
...
//-------------------------------------
void quickSortArrayOfPairs_LAST_2(
    int begin,
    int end,
    vector<Pair>* segment_border_type_array) {
...
//-------------------------------------
void quickSortArrayOfPairs_LAST_3(
    int begin,
    int end,
    vector<Pair>* segment_border_type_array) {
...
//-------------------------------------
int quickSortArrayOfPairs_LAST_simple(int begin, int end, vector<Pair>* array_of_pairs) {
if (begin < end) {
  ...
  if (begin < iterator_last) {
    if (iterator_last - begin > SORT_LIMIT) {
      quickSortArrayOfPairs_LAST_simple(begin, iterator_last, array_of_pairs);
    } else {
      insertionSortArrayOfPairs_LAST(begin, iterator_last, array_of_pairs);
    }
  }
  if (iterator_first < end) {
    if (end - iterator_first > SORT_LIMIT) {
      quickSortArrayOfPairs_LAST_simple(iterator_first, end, array_of_pairs);
    } else {
      insertionSortArrayOfPairs_LAST(iterator_first, end, array_of_pairs);
    }
  }
 }
}
...
/*
//-------------------------------------
int quickSortArrayOfPairs_LAST_4(int begin, int end, vector<Pair>* array_of_pairs) {
  int pivot_index = begin;
  Pair pivot = array_of_pairs->at(pivot_index);
  int iterator_first = begin;
  int iterator_last = end;
  while (iterator_first < iterator_last) {
    while (pivot < array_of_pairs->at(iterator_last) || pivot == array_of_pairs->at(iterator_last)) {
      --iterator_last;
    }
    if (iterator_last > iterator_first) {
      array_of_pairs->at(iterator_first) = array_of_pairs->at(iterator_last);
      while (iterator_first < iterator_last && (array_of_pairs->at(iterator_first) < pivot || 
                                                array_of_pairs->at(iterator_first) == pivot)) {
        ++iterator_first;
      }
      array_of_pairs->at(iterator_last) = array_of_pairs->at(iterator_first);
    }
  }
  array_of_pairs->at(iterator_first) = pivot;
  if (iterator_first - begin > 1) {
    if (iterator_first - begin > SORT_LIMIT) {
      quickSortArrayOfPairs_LAST_simple(begin, iterator_first - 1, array_of_pairs);
    } else {
      insertionSortArrayOfPairs_LAST(begin, iterator_first - 1, array_of_pairs);
    }
  }
  if (end - iterator_last > 1) {
    if (end - iterator_last > SORT_LIMIT) {
      quickSortArrayOfPairs_LAST_simple(iterator_last + 1, end, array_of_pairs);
    } else {
      insertionSortArrayOfPairs_LAST(iterator_last + 1, end, array_of_pairs);
    }
  }
}*/

//-----------------------------------------------------------------------------
int maxNumberOfCoveringSegments_LAST(
    const vector<Pair>& segment_border_type_array, int cover_number_k) {
  int max_number_of_coverage = 0;
  int current_number_of_coverage = 0;
  for (int pair_index = 0; 
...
double solution_LAST(
  ...
  while (right_radius - left_radius > EPSILON) {
    ...
    radius = left_radius + (right_radius - left_radius) / 2.0;
    fillArrayOfSegmentBorderAndType_LAST(
        number_points_n,
        points_array,
        radius,
        &segment_border_type_array);
...
//    cout << radius << endl;
    ...
    quickSortArrayOfPairs_LAST_simple(
        0, 
        segment_border_type_array.size() - 1, 
        &segment_border_type_array);
...
//		Output_Pair(segment_border_type_array);
    ...
    if (maxNumberOfCoveringSegments_LAST(segment_border_type_array, 
        cover_number_k) < cover_number_k) {  // radius is too small
      left_radius = radius;
      continue;
    } else {  // radius is efficient but maybe not minimal
...
while (true) {	
  ...
  vector<Point> points_array;
	vector<Point> points_array_LAST;
	number_points_n = N;
	cover_number_k = N;
	points_array.resize(number_points_n);
	points_array_LAST.resize(number_points_n);
	for (int i = 0; i < number_points_n; ++i) {
	...
	points_array_LAST = points_array;
  double ans = solution(number_points_n, cover_number_k, points_array);
	double ans_LAST = solution_LAST(number_points_n, cover_number_k, points_array_LAST);
//  input(&number_points_n, &cover_number_k, &points_array);
  if (fabs(ans - ans_LAST) >= EPSILON) {
