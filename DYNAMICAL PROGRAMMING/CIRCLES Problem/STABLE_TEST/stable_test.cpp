...
/*****************************************************************************/
//-----------------------------------------------------------------------------
Segment calculateSegment_STABLE(const Point& point, double radius, bool* error_flag) {
  Segment segment;
  if (radius < static_cast<double>(fabs(point.coordinate_Y))) {
    *error_flag = true;
    return segment;
...
//-----------------------------------------------------------------------------
void fillArrayOfSegmentBorderAndType_STABLE(
    int number_points_n,
    const vector<Point>& points_array,
    double radius,
    vector<Pair>* segment_border_type_array) {
  ...
  for (int point_index = 0; point_index < number_points_n; ++point_index) {
      Segment segment_of_current_point = 
          calculateSegment_STABLE(points_array[point_index], radius, &error_flag);
      if (error_flag) {
        error_flag = false;
        continue;
      }
...
//-----------------------------------------------------------------------------
void insertionSortArrayOfPairs_STABLE(
    int begin,
    int end,
    vector<Pair>* segment_border_type_array) {
...
int quickSortArrayOfPairs_simple_STABLE(int begin, int end, vector<Pair>* array_of_pairs) {
if (begin < end) {
  ...
  if (begin < iterator_last) {
    quickSortArrayOfPairs_simple_STABLE(begin, iterator_last, array_of_pairs);
  }
  if (iterator_first < end) {
    quickSortArrayOfPairs_simple_STABLE(iterator_first, end, array_of_pairs);
  }
 }
}

//-----------------------------------------------------------------------------
int maxNumberOfCoveringSegments_STABLE(
    const vector<Pair>& segment_border_type_array, int cover_number_k) {
  int max_number_of_coverage = 0;
  int current_number_of_coverage = 0;
  for (int pair_index = 0; 
...
double solution_STABLE(
  ...
  while (right_radius - left_radius > EPSILON) {
    ...
    radius = left_radius + (right_radius - left_radius) / 2.0;
    fillArrayOfSegmentBorderAndType_STABLE(
        number_points_n,
        points_array,
        radius,
        &segment_border_type_array);
 
    quickSortArrayOfPairs_simple_STABLE(
        0, 
        segment_border_type_array.size() - 1, 
        &segment_border_type_array);
...
//		Output_Pair(segment_border_type_array);
    ...
    if (maxNumberOfCoveringSegments_STABLE(segment_border_type_array, 
        cover_number_k) < cover_number_k) {  // radius is too small
      left_radius = radius;
      continue;
    } else {  // radius is efficient but maybe not minimal
...
while (true) {	
  ...
  double ans = solution(number_points_n, cover_number_k, points_array);
	double ans_LAST = solution_STABLE(number_points_n, cover_number_k, points_array_LAST);
//  input(&number_points_n, &cover_number_k, &points_array);
  if ((double)clock() / CLOCKS_PER_SEC - clockverk >= 1.000) {
	  output(ans);
	  cout << " " << number_points_n << " " << cover_number_k << "  TLE!!!: " << 
