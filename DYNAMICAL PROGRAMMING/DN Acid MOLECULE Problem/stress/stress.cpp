...
void multiplyWithFFT(
  ...
  calculateFFT(false, &fourier_first_polynomial);
  ...
  calculateFFT(true, &fourier_first_polynomial);
...
int main() {
  ...
  while (true) {
    ...
    int first_length = static_cast<double>(rand()) / RAND_MAX * (LENGTH - 1) + 1;
    ...
    cout << static_cast<double>(slow_time) / CLOCKS_PER_SEC << "  ";
    if (/*answer == naive_answer &&*/ answer == slow_answer) {
      cout << "OK";
    } else {
      cout << "WRONG ANSWER !!!\a";
    }
    cout << "  " << answer << " " << slow_answer /*<< " " << naive_answer*/ << "  ";
