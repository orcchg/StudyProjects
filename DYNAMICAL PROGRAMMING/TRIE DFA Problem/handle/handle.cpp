...
struct TrieNode {
  ...
  friend ostream& operator << (ostream& out, const TrieNode& obj) {
    out << obj.index_in_trie << " " << obj.parent_node << " " << obj.label_from_parent << " " << obj.suffix_reference;
    return out;
  }
};
...
vector<TrieNode> makeTrieFromStrings(
  ...
  for (int index = 0; index < forbidden_strings.size(); ++index) {
    ...
    trie[node_index].is_terminal = true;  // as end of forbidden string
    trie[node_index].is_forbidden = true;
  }
  return trie;
}
...
void printTrie(const vector<TrieNode>& trie) {
  for (int node_index = 0; node_index < trie.size(); ++node_index) {
    cout << "NODE: " << node_index << " " << trie[node_index].index_in_trie << "  suffix: " <<
        trie[node_index].suffix_reference << " ";
    if (trie[node_index].is_terminal) {
      cout << " TERMINAL";
    }
    cout << endl;
    for (int edge_index = 0; 
        edge_index < trie[node_index].transitions.size(); 
        ++edge_index
        ) {
      char ch = 'a' + edge_index;
      cout << "    " << edge_index << "[" << ch << ": ";
      if (trie[node_index].transitions[edge_index] != -1) {
        cout << trie[node_index].transitions[edge_index]; 
      }
      cout << "]  (" << trie[node_index].transitions[edge_index] << 
          ";" << ch << ") " << endl;
    }
  ...
  while (length <= string_length) {
    ...
    for (int index = 0; index < non_terminal_nodes.size(); ++index) {
      for (int edge_index = 0; edge_index < alphabet_size; ++edge_index) {
        ...
        cout << "--> " << length << " " << index << " " << non_terminal_nodes[index].index_in_trie << " " <<label << " " << destination << " ";
        if (!dfa_on_trie[destination].is_terminal) {
          int from_index = non_terminal_nodes[index].index_in_trie;
          next_number_of_accepts[destination] += 
              previous_number_of_accepts[from_index] % MODULUS;
          next_number_of_accepts[destination] %= MODULUS;
          cout << next_number_of_accepts[destination];
        }
        cout << endl;
      }
    }
    previous_number_of_accepts = next_number_of_accepts;
    copy(previous_number_of_accepts.begin(), previous_number_of_accepts.end(), ostream_iterator<int>(cout, " "));
    cout << endl;
    ++length;
  }
  long long result_number_of_strings = 0;
