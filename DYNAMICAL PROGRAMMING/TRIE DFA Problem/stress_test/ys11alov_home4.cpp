...
void inputStress(const string& filename, vector<string>* test) {
...
void makeDFAfromTrie(
  ...
  for (int node_index = 0; node_index < dfa_on_trie->size(); ++node_index) {
    if (dfa_on_trie->at(node_index).is_terminal) {
      continue;
    }
    ...
    if (back_reference != EDGE_IS_ABSENT && 
        ...
        dfa_on_trie->at(back_reference).is_terminal
        ) {
...
int main(int argc, char** argv) {
  ...
  vector<string> stress_test;
  inputStress(filename, &stress_test);
  vector<string> forbidden_strings;
  ...
  cout << "FAST: " << fast_answer << "  NAIVE: " << naive_answer << " ";
