...
bool isSubstring(const string& text, const string& pattern) {
  int kmp_result = KMP(text, 0, pattern);
  if (kmp_result != -1) {
    return true;
  }
  return false;
...
int independentSlowSolution(
  ...
  for (vector<string>::const_iterator it = strings.begin();
    ...
    int counter = 0;
	for (vector<string>::const_iterator sub = forbidden_strings.begin();
        sub != forbidden_strings.end();
        ++sub, ++counter
        ) {
      if (isSubstring(*it, *sub)) {
        break;
      }
    }
    if (counter == forbidden_strings.size()) {
      ++answer;
      answer %= MODULUS;
    }
  }
...
while (true) {
  clock_t rest = clock();
  int alphabet_size = rand() % ALPHA + 1;
  int string_length = rand() % LENGTH + 1;
  int number_of_forbidden = rand() % (FORBID + 1);  // 0 is allowed
  vector<string> forbidden_strings = generateForbidden(number_of_forbidden, alphabet_size, string_length);
  ...
  clock_t slow_time = 0;
  int naive_answer = independentSlowSolution(stress_test, forbidden_strings); // naiveSolution(stress_test, dfa_on_trie);
  ...
  if (fast_answer == naive_answer) {
    ...
    cout << endl << stress_test.size() << endl;
    copy(forbidden_strings.begin(), forbidden_strings.end(), ostream_iterator<string>(cout, "\n"));
    ++wa;
    break;
  }
