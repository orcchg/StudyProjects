...
vector<TrieNode> makeTrieFromStrings(
  ...
  for (int index = 0; index < forbidden_strings.size(); ++index) {
    ...
    trie[node_index].is_terminal = true;
  }
  return trie;
}
...
int makeDFAtransition(
  ...
  if (trie->at(from_node).transitions[label_index] == EDGE_IS_ABSENT) {
    if (trie->at(from_node).next_node[label_index] != EDGE_IS_ABSENT) {
      trie->at(from_node).transitions[label_index] = 
          trie->at(from_node).next_node[label_index];
    } else {
...
void makeDFAfromTrie(
  ...
  for (int node_index = 0; node_index < dfa_on_trie->size(); ++node_index) {
    if (dfa_on_trie->at(node_index).is_forbidden ||
        dfa_on_trie->at(node_index).is_terminal
        ) {
      continue;
    }
    int back_reference = dfa_on_trie->at(node_index).suffix_reference;
    if (back_reference != EDGE_IS_ABSENT && 
        back_reference != TRIE_ROOT_INDEX && 
        dfa_on_trie->at(back_reference).is_forbidden
        ) {
...
int calculateNumberOfAccepts(
  ...
  int length = 1;
  vector<int> previous_number_of_accepts(dfa_on_trie.size(), 0);
  previous_number_of_accepts[0] = 1;  // value for root (empty accepted string)
  while (length <= string_length) {
    vector<int> next_number_of_accepts(dfa_on_trie.size(), 0);
  ...
  for (vector<int>::const_iterator it = previous_number_of_accepts.begin();
