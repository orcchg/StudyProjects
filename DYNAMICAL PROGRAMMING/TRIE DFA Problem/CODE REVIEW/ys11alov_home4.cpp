...
int getIndexFromCharacter(char current_character) {
  return current_character - 'a';
}

char getCharacterFromIndex(int character_index) {
  return character_index + 'a';
}
...
vector<TrieNode> makeTrieFromStrings(
  ...
  for (int index = 0; index < forbidden_strings.size(); ++index) {
    ...
    for (int char_index = 0;
      ...
      int current_char = getIndexFromCharacter(forbidden_strings[index][char_index]);
...
bool isTerminal(int node_index, const vector<TrieNode>& dfa_on_trie);
...
void makeDFAfromTrie(
  ...
  for (int node_index = 0; node_index < dfa_on_trie->size(); ++node_index) {
    for (int char_index = 0; char_index < alphabet_size; ++char_index) {
      char label = getCharacterFromIndex(char_index);
      makeDFATransition(node_index, label, dfa_on_trie);
    }
  }
  for (int node_index = 0; node_index < dfa_on_trie->size(); ++node_index) {
    if (isTerminal(node_index, *dfa_on_trie)) {
...
bool isTerminal(
  ...
  return isTerminal(back_reference, dfa_on_trie);
}
...
int calculateNumberOfAccepts(
  ...
  previous_number_of_accepts[TRIE_ROOT_INDEX] = 1;
    ...
    for (int index = 0; index < non_terminal_nodes.size(); ++index) {
      for (int edge_index = 0; edge_index < alphabet_size; ++edge_index) {
        ...
        if (!dfa_on_trie[destination].is_terminal) {
          next_number_of_accepts[destination] = (next_number_of_accepts[destination] + 
              previous_number_of_accepts[non_terminal_nodes[index].index_in_trie]) % MODULUS;
        }
      }
    }
  ...
  for (vector<long long>::const_iterator it = previous_number_of_accepts.begin();
    ...
    if (*it != 0) {
      result_number_of_strings = (result_number_of_strings + *it) % MODULUS;
    }
  }
