...
bool isTerminalRecoursiveDefine(int node_index, const vector<TrieNode>& dfa_on_trie);
...
void makeDFAfromTrie(
  ...
  for (int node_index = 0; node_index < dfa_on_trie->size(); ++node_index) {
    if (isTerminalRecoursiveDefine(node_index, *dfa_on_trie)) {
...
bool isTerminalRecoursiveDefine(
    int node_index, 
  ...
  return isTerminalRecoursiveDefine(back_reference, dfa_on_trie);
}
...
int calculateNumberOfAccepts(
  ...
  previous_number_of_accepts[0] = 1;
